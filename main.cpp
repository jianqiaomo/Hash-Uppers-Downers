 ...
 */
int ConnectSocket()
{ // nc uppers-downers.hack.fe-ctf.dk 1337
    // socket
                   ...
                   unsigned char *low, unsigned char *high)
{
    ...
    if (ServerCmp(client, SortedHashSampleArray[0].passw) == '>')
    { // good < user[0]
        memcpy(high, SortedHashSampleArray[0].userh, sizeof(high));
        return;
    }
    else if (ServerCmp(client, SortedHashSampleArray[ArrayLen - 1].passw) == '<')
    { // user[largest] < good
...
int main()
{
    ...
    for (unsigned j = 0; j < 3; ++j)
    {
        ...
        if (j == 1)
        {
            if (memcmp(SocketBuf + 6, salt, sizeof(salt) - 1) != 0)
            { // get your salt
